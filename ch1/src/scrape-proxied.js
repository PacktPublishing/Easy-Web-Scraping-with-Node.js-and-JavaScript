...
const scrape = (url, maxPages = 10) => {
  let proxies = [];
  try {
    // read the proxies.txt file into a string
    proxies = fs.readFileSync("proxies.txt", "utf8");
    // make an array out of the proxies string
    proxies = proxies.split("\n");
    // filter out empty proxies
    proxies = proxies.filter((proxy) => proxy.length > 0);
  } catch (err) {
    console.error("Could not read proxies", err);
  }
